...
service_servergroup = Table('service_servergroup' , metadata , \
        Column('id' ,INTEGER , primary_key = True),\
...
class TaskStatus (Declbase , Base) :
    ...
    def __repr__(self,):
        return "task_id=%s,success_arry=%s,key_code_list=p_key_code_list"%(self.task_id,self.success_arry,self.key_code_list)


class Service(Declbase,Base):
    ...
    def __init__(self , p_name ,p_desc):
        ...
        self.desc = p_desc
    def __repr__(self):
        return 'name=%s , desc=%s ' % (self.name  , self.desc)
    def dump(self) :
        ret = {}
        ret['id'] = self.id 
        ret['name'] = self.name
